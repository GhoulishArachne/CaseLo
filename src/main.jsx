...
const themeColors = [
  ...
  },
  {
    ...
    reportHeaderText: "Internal Affairs Investigation",
    signatureBlockText: "Authorized by: ",
    accentSecondaryColor: "#FFFFFF",
  },
  {
    name: "FiveM Premium MDT",
    dark: "#0A0F1C",
    accent: "#2563EB",
    departmentName: "Police Department",
    departmentLogoUrl: "",
...
async function createPerson(event) {
  ...
  // Executive Command Center layout for Department Coordinator theme
  const isExecutiveTheme = themeColors[themeIndex]?.name === "Department Coordinator Executive Suite";

  if (isExecutiveTheme) {
    return (
      <main className="shell" style={{ display: "grid", gridTemplateColumns: "240px 1fr 320px", gridTemplateRows: "80px 1fr", minHeight: "100vh" }}>
        {/* Left Sidebar - Quick Navigation */}
        <aside style={{ gridColumn: 1, gridRow: "1/3", background: "#1E293B", padding: 16, borderRight: "1px solid #334155", overflowY: "auto" }}>
          <div style={{ marginBottom: 24 }}>
            <p style={{ margin: "0 0 12px 0", fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase" }}>Navigation</p>
            <div style={{ display: "grid", gap: 8 }}>
              {["Dashboard", "Cases", "People", "Complaints", "Settings"].map((item) => (
                <button
                  key={item}
                  ...
                  style={{
                    ...
                    padding: "10px 12px",
                    borderRadius: 6,
                    background: item === activeView ? "#0F172A" : "transparent",
                    color: item === activeView ? "#94A3B8" : "#64748B",
                    border: item === activeView ? "1px solid #334155" : "1px solid transparent",
                    cursor: "pointer",
                    fontSize: 13,
                    fontWeight: 500,
                    transition: "all 0.2s",
          ...
          <div style={{ marginBottom: 24, paddingTop: 16, borderTop: "1px solid #334155" }}>
            <p style={{ margin: "0 0 12px 0", fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase" }}>Quick Stats</p>
            <div style={{ display: "grid", gap: 12 }}>
              <div style={{ background: "#0F172A", padding: 12, borderRadius: 6, borderLeft: "3px solid #94A3B8" }}>
                <p style={{ margin: 0, fontSize: 11, color: "#64748B" }}>Open Cases</p>
                <p style={{ margin: "4px 0 0 0", fontSize: 20, fontWeight: 700, color: "#F8FAFC" }}>{data.cases?.filter(c => c.status !== "Closed").length || 0}</p>
              </div>
              <div style={{ background: "#0F172A", padding: 12, borderRadius: 6, borderLeft: "3px solid #94A3B8" }}>
                <p style={{ margin: 0, fontSize: 11, color: "#64748B" }}>Personnel</p>
                <p style={{ margin: "4px 0 0 0", fontSize: 20, fontWeight: 700, color: "#F8FAFC" }}>{data.people?.length || 0}</p>
              </div>
              <div style={{ background: "#0F172A", padding: 12, borderRadius: 6, borderLeft: "3px solid #94A3B8" }}>
                <p style={{ margin: 0, fontSize: 11, color: "#64748B" }}>Pending Complaints</p>
                <p style={{ margin: "4px 0 0 0", fontSize: 20, fontWeight: 700, color: "#F8FAFC" }}>{data.complaints?.filter(c => c.status !== "Closed").length || 0}</p>
              </div>
            </div>
          </div>
        </aside>
        ...
        {/* Top Header */}
        <header style={{ gridColumn: "2/4", gridRow: 1, background: "#1E293B", borderBottom: "1px solid #334155", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div>
              ...
              <p style={{ margin: "4px 0 0 0", fontSize: 12, color: "#94A3B8" }}>Executive Command Center</p>
            </div>
          </div>
          <div className="searchbox" style={{ background: "#0F172A" }}>
            <Search size={18} color="#94A3B8" />
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search cases, personnel, complaints..."
              style={{ background: "transparent", color: "#F8FAFC", borderColor: "#334155" }}
            />
          </div>
        </header>

        {/* Main Content Area */}
        <section style={{ gridColumn: 2, gridRow: 2, overflow: "auto", background: "#0F172A", padding: 24 }}>
          {activeView === "Dashboard" && (
            <div>
              {/* KPI Cards */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
                <div style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 8, padding: 16 }}>
                  <p style={{ margin: 0, fontSize: 12, color: "#94A3B8" }}>OPEN CASES</p>
                  <p style={{ margin: "8px 0 0 0", fontSize: 28, fontWeight: 700, color: "#2563EB" }}>{data.cases?.filter(c => c.status !== "Closed").length || 0}</p>
                </div>
                <div style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 8, padding: 16 }}>
                  <p style={{ margin: 0, fontSize: 12, color: "#94A3B8" }}>PENDING COMPLAINTS</p>
                  <p style={{ margin: "8px 0 0 0", fontSize: 28, fontWeight: 700, color: "#F59E0B" }}>{data.complaints?.filter(c => c.status !== "Closed").length || 0}</p>
                </div>
                <div style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 8, padding: 16 }}>
                  <p style={{ margin: 0, fontSize: 12, color: "#94A3B8" }}>ACTIVE FINDINGS</p>
                  <p style={{ margin: "8px 0 0 0", fontSize: 28, fontWeight: 700, color: "#EF4444" }}>{data.findings?.filter(f => f.status !== "Closed").length || 0}</p>
                </div>
                <div style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 8, padding: 16 }}>
                  <p style={{ margin: 0, fontSize: 12, color: "#94A3B8" }}>PERSONNEL</p>
                  <p style={{ margin: "8px 0 0 0", fontSize: 28, fontWeight: 700, color: "#10B981" }}>{data.people?.length || 0}</p>
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                <div style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 8, padding: 16 }}>
                  <h3 style={{ margin: "0 0 12px 0", fontSize: 14, fontWeight: 700, color: "#F8FAFC" }}>Recent Cases</h3>
                  <div style={{ display: "grid", gap: 8, maxHeight: "400px", overflowY: "auto" }}>
                    {filteredCases.slice(0, 5).map((c) => (
                      ...
                      >
                        <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: "#2563EB" }}>{c.id}</p>
                        <p style={{ margin: "4px 0 0 0", fontSize: 11, color: "#94A3B8" }}>Status: {c.status}</p>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 8, padding: 16 }}>
                  <h3 style={{ margin: "0 0 12px 0", fontSize: 14, fontWeight: 700, color: "#F8FAFC" }}>Recent Complaints</h3>
                  <div style={{ display: "grid", gap: 8, maxHeight: "400px", overflowY: "auto" }}>
                    {visibleRecords.complaints.slice(0, 5).map((c) => (
                      <button
                        key={c.id}
                      ...
                      >
                        <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: "#F59E0B" }}>{c.id}</p>
                        <p style={{ margin: "4px 0 0 0", fontSize: 11, color: "#94A3B8" }}>Status: {c.status || "Submitted"}</p>
                      </button>
                    ))}
                  </div>
                </div>
        ...
        <aside style={{ gridColumn: 3, gridRow: 2, background: "#1E293B", borderLeft: "1px solid #334155", padding: 16, overflowY: "auto" }}>
          <p style={{ margin: "0 0 12px 0", fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase" }}>Active Alerts</p>
          <div style={{ display: "grid", gap: 12 }}>
            {data.findings?.filter(f => f.status !== "Closed").slice(0, 5).map((f, idx) => (
              <div key={idx} style={{ background: "#0F172A", border: "1px solid #ef4444", borderRadius: 6, padding: 12 }}>
                <p style={{ margin: 0, fontSize: 11, fontWeight: 600, color: "#EF4444" }}>FINDING</p>
                <p style={{ margin: "4px 0 0 0", fontSize: 12, color: "#F8FAFC" }}>{f.id || "ID-" + idx}</p>
              </div>
            ))}
            {(!data.findings || data.findings.filter(f => f.status !== "Closed").length === 0) && (
              <p style={{ margin: 0, fontSize: 12, color: "#64748B" }}>No active alerts</p>
            )}
          </div>

          <p style={{ margin: "24px 0 12px 0", fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase" }}>Risk Flags</p>
          <div style={{ display: "grid", gap: 8 }}>
            {Object.entries(earlyInterventionByEmployeeId || {})
              .filter(([_, data]) => data.flags?.length > 0)
              .slice(0, 3)
              .map(([personId, data]) => (
                <div key={personId} style={{ background: "#0F172A", border: "1px solid #F59E0B", borderRadius: 6, padding: 10 }}>
                  <p style={{ margin: 0, fontSize: 11, fontWeight: 600, color: "#F59E0B" }}>{data.flags.length} FLAG{data.flags.length !== 1 ? "S" : ""}</p>
                  <p style={{ margin: "4px 0 0 0", fontSize: 11, color: "#94A3B8" }}>{data.summary?.split(" ").slice(0, 3).join(" ")}</p>
                </div>
              ))}
          </div>
        </aside>
